...
export function computeConnectionPoints(x,y,width, height) {
  ...
  export function generateEdgePath(edge) {
    ...
    const sourceConnectionPoints = computeConnectionPoints(edge.x1, edge.y1, sourceNode.data.width, sourceNode.data.height);
    console.log("    Source Connection Points:", sourceConnectionPoints);
    const targetConnectionPoints = computeConnectionPoints(edge.x2, edge.y2, targetNode.data.width, targetNode.data.height);
    ...
    if ((sourcePoint.side == ConnectorSide.LEFT && targetPoint.side == ConnectorSide.RIGHT) || 
        ( sourcePoint.side == ConnectorSide.RIGHT && targetPoint.side == ConnectorSide.LEFT) ) { // left to right or right to left
    ...
    else if ((sourcePoint.side == ConnectorSide.BOTTOM && targetPoint.side == ConnectorSide.TOP) || 
        ( sourcePoint.side == ConnectorSide.TOP && targetPoint.side == ConnectorSide.BOTTOM )) { // bottom to top or top to bottom 
      if (edge.settings.curved) {
        waypoints = [
          // // for a curve
